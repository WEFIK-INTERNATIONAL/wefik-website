...
function DriftingPath({ d, index }) {
    const x = useMotionValue(0);
    const y = useMotionValue(0);
    const stroke = useMotionValue("rgba(80,80,80,0.35)");

    useAnimationFrame((t) => {
        const percent = (1 - Math.sin(index * 0.35 + 0.0022 * t)) / 2;
        ...
        // brighter lime-green vs dim gray
        const r = Math.round(187 + (80 - 187) * percent);
        const g = Math.round(244 + (80 - 244) * percent);
        const b = Math.round(81 + (80 - 81) * percent);
        const a = 1 + (0.35 - 1) * percent;
        stroke.set(`rgba(${r}, ${g}, ${b}, ${a})`);
    });
...
export default function SphereAnimation() {
    return (
        <div className="flex items-center justify-center w-full h-1/2 absolute">
            <motion.div className="relative w-full max-w-[580px] aspect-square">
                <motion.svg
                    viewBox="0 0 440 440"
                ...
                >
                    ...
                    >
                        {paths.map((d, i) => (
                            <DriftingPath key={i} d={d} index={i} />
                        ))}
                    </motion.g>
                </motion.svg>
            </motion.div>
